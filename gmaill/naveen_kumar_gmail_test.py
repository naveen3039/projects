...
#Sending to trash with respective message id
      results= service.users().messages().trash(userId='me',id='179cdd4671b43ae6').execute()
      logging.info("message sented to trash")
      result = service.users().messages().list(userId='me').execute()
      ids=result.get('messages', [])
      for i in ids:
          assert i['id'] != '179cdd4671b43ae6'

def test_untrash():
#Untrashing message with respective message id
      results= service.users().messages().untrash(userId='me',id='179cdd4671b43ae6').execute()
      logging.info(" untrashed message with respective message id")
      result = service.users().messages().list(userId='me').execute()
      ids=result.get('messages', [])
      for i in ids:
        if i['id'] == '179cdd4671b43ae6':
            assert i['id'] == '179cdd4671b43ae6'

def test_delete():
#Deleting message respective message id
    results= service.users().messages().delete(userId='me',id ='179cdd4671b43ae6').execute()
